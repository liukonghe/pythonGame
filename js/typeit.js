...
function dfFun() {
    if (dy)
    {
		var bbb=$('.CodeMirror-code')
        var aaa=$(editor.display.input)
        aaa.typeIt({
            ...
            editor.setValue(editor.getValue()+dyText);
        });
 ...
 (function($){
   ...
   $.fn.typeIt.typeItClass = function(theElement, options, callback){
    ...
    // callback function that executes after strings have been printed：在打印完字符串后执行的回调函数
